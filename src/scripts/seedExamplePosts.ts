...
const examplePosts: Omit<Post, 'id' | 'datePosted' | 'isAdminPost'>[] = [
  // Lost Item Example
  {
...
export const seedExamplePosts = async (): Promise<void> => {
  ...
  try {
    for (const postData of examplePosts) {
      const post: Post = {
        ...
        datePosted: new Date(),
        isAdminPost: true // Mark as admin post for watermarking
      };
      
      await createPost(post);
      console.log(`Created example post: ${post.title}`);
    }
